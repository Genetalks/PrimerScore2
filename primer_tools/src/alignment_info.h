...
public:
    ...
    void hardclip_by_primer_end(int32_t pe);
...
typedef IntervalTree<int32_t, alignment_info_ptr> interval_tree_t;
...
public:
    ...
    interval_vector_t query(int32_t primer_start, int32_t primer_end) const {   
        auto v = this->query_index->findOverlapping(primer_start, primer_end);
        return v;
    }
